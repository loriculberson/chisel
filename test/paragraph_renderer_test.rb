...
class ParagraphRendererTest < Minitest::Test 
  ...
  def test_can_convert_asterisk_to_emphasis_tag
    ...
    assert_equal "You just <em>have</em> to try the cheesecake!", p_renderer.word_format
  end
  ...
  def test_can_convert_two_asterisks_to_strong_tag
    ...
    paragraph = "You just have to try the **cheesecake!**"
    p_renderer = ParagraphRenderer.new(paragraph)
    assert_equal "You just have to try the <strong>cheesecake!</strong>", p_renderer.word_format
  end
  ...
  def test_can_convert_to_emphasis_and_strong_tag
    ...
    paragraph = "You just *have* to try the **cheesecake!**"
    p_renderer = ParagraphRenderer.new(paragraph)
    assert_equal "You just <em>have</em> to try the <strong>cheesecake!</strong>", p_renderer.word_format
  end
...
DOCUMENT
    ...
    formatted_text = '"Ever since it appeared in <strong>Food &amp; Wine</strong> this place has been packed every night."'
    p_renderer = ParagraphRenderer.new(paragraph)
